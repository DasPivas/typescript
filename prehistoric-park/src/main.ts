...
class App implements AppApi {
  ...
  async start(): Promise<void> {
    ...
    this.startLevel(this.hasSave() ? 'ravnina' : 'ravnina');
    this.ui.openLevels();
  ...
  private makeIcons(): void {
    for (const def of ALL_DEFS) {
      const tex = this.scene.art.buildings[def.key];
      const src =
        def.key === 'road_dirt'
...
Object.assign(window as unknown as Record<string, unknown>, { pp: app, ENTRY, MAP_H });
